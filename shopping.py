...
def load_data(filename):
    ...
    integer = ['Administrative','Informational','ProductRelated','Month','OperatingSystems','Browser','Region','TrafficType','VisitorType','Weekend']
    floats = ['SpecialDay','PageValues','ExitRates','BounceRates','ProductRelated_Duration','Informational_Duration','Administrative_Duration']
    floats = reversed(floats)
    for v in integer:
        if data[v].dtype!='int64':
            data = data.astype({v:'int64'})
    for v in floats:
        if data[v].dtype!='float64':
            data = data.astype({v:'float64'})
...
def evaluate(labels, predictions):
    ...
    for i in range(len(labels)):
        if labels[i]==predictions[i]==1:
            ...
            spec+=1
    sensitivity = sens/labels.count(1)
    specifity = spec/labels.count(0)
